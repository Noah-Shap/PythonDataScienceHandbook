...
Generated: 2026-09-16T01:48:38+00:00 | criteria_version: 1
...
|---|------|-------|-----------|
| 6.1 | Formal foundations (`formal`) | 30 | 47 |
| 6.2 | Argument mining (NLP) (`mining`) | 60 | 128 |
| 6.3 | Argument quality and evaluation (`quality`) | 40 | 66 |
| 6.4 | Dialogue and debate (`dialogue`) | 40 | 50 |
| 6.5 | LLM era, 2023-2026 (`llm`) | 40 | 51 |
| 6.6 | Datasets, tools, annotation guidelines (`resources`) | 30 | 77 |

### 6.1 Formal foundations  `formal`  (quota >= 30, currently 47)
...
### 6.2 Argument mining (NLP)  `mining`  (quota >= 60, currently 128)
...
### 6.3 Argument quality and evaluation  `quality`  (quota >= 40, currently 66)
...
### 6.4 Dialogue and debate  `dialogue`  (quota >= 40, currently 50)
...
### 6.5 LLM era, 2023-2026  `llm`  (quota >= 40, currently 51)
...
### 6.6 Datasets, tools, annotation guidelines  `resources`  (quota >= 30, currently 77)
...
Supplementary per-area queries, added so the quotas in under-served areas have a fair chance (logged separately in the run report):

- **formal**: `abstract argumentation semantics`, `structured argumentation ASPIC`, `assumption-based argumentation`, `defeasible logic programming argumentation`, `argumentation framework acceptability`, `Toulmin model of argument`, `argumentation scheme critical questions`
- **mining**: `argument component classification`, `argument relation identification`, `end-to-end argument mining`, `argumentative discourse unit segmentation`, `cross-domain argument mining`
- **quality**: `argument convincingness prediction`, `argument strength prediction`, `argumentation quality dimensions`, `fallacy classification argumentation`
- **dialogue**: `dialogical argumentation corpus`, `broadcast debate argument corpus`, `burden of proof dialogue`, `online debate persuasion corpus`, `argument mining in dialogue`
- **llm**: `large language models argument quality`, `LLM fallacy detection`, `argument generation large language model`, `prompting for argument mining`, `LLM as a judge argument`
- **resources**: `argument annotation scheme corpus`, `AIFdb argument corpus`, `argument search engine corpus`, `argument quality dataset`

## Tier rubric

- **T1** - Foundational / survey. Read in full: theory anchors, major surveys, canonical papers. (14 in corpus)
- **T2** - Core method. Defines a task formulation, model, or evaluation still in use. (66 in corpus)
- **T3** - Dataset / tool / annotation guideline, including the guideline documents themselves. (35 in corpus)
- **T4** - Recent (2023-2026). LLM-era work; lower durability confidence, high build relevance. (9 in corpus)
